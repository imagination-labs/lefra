...
// Allow letters (any case), digits, underscores, and hyphens; must start/end with a letter or digit.
const VALID_NAME_REGEX = /^[\p{L}\d](?:[\p{L}\d_-]*[\p{L}\d])?$/u;
  ...
  protected static validateName(name: string) {
    ...
    if (!VALID_NAME_REGEX.test(name)) {
      throw new LedgerAccountError(
        `Account name is invalid. Use letters, digits, underscores, or hyphens, and start/end with a letter or digit. Name: ${name}`,
      );
    }
  }
}
