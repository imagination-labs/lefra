...
// Allow uppercase letters, digits, underscores, and hyphens; must start/end with a letter or digit.
const VALID_NAME_REGEX = /^[\p{Lu}\d](?:[\p{Lu}\d_-]*[\p{Lu}\d])?$/u;
  ...
  protected static validateName(name: string) {
    ...
    if (!VALID_NAME_REGEX.test(name)) {
      throw new LedgerAccountError(
        `Account name is invalid. Use uppercase letters, digits, underscores, or hyphens, and start/end with a letter or digit. Name: ${name}`,
      );
    }
  }
}
